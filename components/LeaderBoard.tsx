...
function LeaderBoard({leaderboard}: LeaderBoardProps) {
  ...
  const formattedName = firstname.charAt(0).toUpperCase() + firstname.slice(1).toLowerCase();
    return (
      ...
      >
        <h1  className="text-white text-2xl text-center ">{rank}</h1>
        <h1 className="text-white text-2xl text-center">{formattedName}</h1>
    ...
    return (
        <div style={{
            fontFamily: pixelify.style.fontFamily}} className='w-[30vw] h-[90] bg-neutral-800 border-gray-300 border-l-2'>
         <div className='mt-3'>
           <h1 className='text-green-400 text-5xl underline text-center'>GeeksBoard</h1>
         </div>

        <div className=' overflow-y-auto h-[70vh]'>
           {leaderboard.map((entry ,index) => displayPosition(entry.rank, entry.name, entry.points ,index))}
        </div>
